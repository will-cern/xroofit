...
std::shared_ptr<const RooFitResult> xRooFit::minimize(RooAbsReal& nll, const std::shared_ptr<ROOT::Fit::FitConfig>& _fitConfig) {
    ...
    for(int tries=1,maxtries=4; tries <= maxtries; ++tries) {
        ...
        if (tries==1 && actualFirstMinimizer != _minimizer.fitter()->Config().MinimizerType() ) actualFirstMinimizer = _minimizer.fitter()->Config().MinimizerType();
        ...
        if (_minimizer.fitter()->Result().Status() == 4 && minim!="Minuit") {
            if(printLevel >= -1) Warning("fitTo","%s Hit max function calls of %d",fitName.Data(),_minimizer.fitter()->Config().MinimizerOptions().MaxFunctionCalls());
            if(autoMaxCalls) {
                if(printLevel >= -1) Warning("fitTo", "will try doubling this");
                _minimizer.fitter()->Config().MinimizerOptions().SetMaxFunctionCalls(_minimizer.fitter()->Config().MinimizerOptions().MaxFunctionCalls()*2);
                _minimizer.fitter()->Config().MinimizerOptions().SetMaxIterations(_minimizer.fitter()->Config().MinimizerOptions().MaxIterations()*2);
        ...
        if(printLevel >= -1) Warning("fitTo","%s Status=%d (edm=%f, tol=%f, strat=%d), Rescanning #%d...",fitName.Data(),status,_minimizer.fitter()->Result().Edm(),
    ...
    RooFitResult* out = _minimizer.save(fitName,resultTitle);
    ...
    if (boundaryCheck) {
        ...
        while ((a = itr.next())) {
            ...
            if (v->getMin() > v->getVal() - vRange * boundaryCheck || v->getMax() < v->getVal() + vRange * boundaryCheck) {
                // within 0.01% of edge
