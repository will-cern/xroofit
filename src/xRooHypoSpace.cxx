...
int xRooNLLVar::xRooHypoSpace::AddPoints(const char *parName, size_t nPoints, double low, double high)
{
   ...
   if(low < _par->getMin()) {
      Warning("AddPoints","low edge of hypoSpace %g below lower bound of parameter: %g. Changing to lower bound",low,_par->getMin());
      low = _par->getMin();
   }
   if(high > _par->getMax()) {
      Warning("AddPoints","high edge of hypoSpace %g above upper bound of parameter: %g. Changing to upper bound",high,_par->getMax());
      high = _par->getMax();
   }
