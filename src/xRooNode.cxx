...
xRooNode::xRooNode(const char* classname, const char* name, const char* title) :
    xRooNode(name,std::shared_ptr<TObject>(TClass::GetClass( classname ) ? (TObject*)TClass::GetClass( classname )->New() : nullptr,
            [](TObject* o) {
    ...
    if (auto _ws = get<RooWorkspace>(); _ws && (!parent || parent->get<TFile>())) {
        ...
        for(auto f : *gROOT->GetListOfFiles()) {
            if ((dynamic_cast<TFile*>(f)->GetVersion()/100) > (gROOT->GetVersionInt()/100)) {
                Warning("xRooNode", "There is file open with version %d > current version %d ... results may be wrong",dynamic_cast<TFile*>(f)->GetVersion(),gROOT->GetVersionInt());
            }
        }
...
xRooNode::xRooNode(const std::shared_ptr<TObject>& comp, const std::shared_ptr<xRooNode>& parent) : xRooNode(
        [&]() {
...
void xRooNode::Browse(TBrowser* b) {
    ...
    if (b==0) {
        ...
        if (!b2 || !b2->GetBrowserImp()) { // no browser imp if browser was closed
            ...
            auto _b = dynamic_cast<TGFileBrowser *>( dynamic_cast<TRootBrowser *>(b2->GetBrowserImp())->fActBrowser );
    ...
    if(auto item = GetTreeItem(b); item) {
        ...
        if (auto first = item->GetFirstChild()) {
            do {
                if (first->HasCheckBox()) {
                    auto _obj = static_cast<xRooNode *>(first->GetUserData());
                    first->CheckItem(_obj->get() && (_obj->get()->TestBit(1 << 20)||(_obj->get<RooAbsArg>() && !_obj->get<RooAbsArg>()->getAttribute("hidden"))));
                }
            } while( (first = first->GetNextSibling()) );
        }
    }
...
bool xRooNode::IsFolder() const {
    ...
    if (strlen(GetName())>0 && GetName()[0]=='.') return true;
    if(empty()) const_cast<xRooNode*>(this)->browse();
    return !empty();
}
...
public:
    ...
    void 	Set (Int_t nbins, const Double_t *xbins) override { if(auto v = dynamic_cast<RooRealVar*>(rvar())) v->setBinning(RooBinning(nbins,xbins),GetName()); TAxis::Set(nbins,xbins); }
    void 	Set (Int_t nbins, const Float_t *xbins) override { std::vector<double> bins(nbins+1); for(int i=0;i<=nbins;i++) bins.at(i)=xbins[i]; return Set(nbins,&bins[0]); }
    void 	Set (Int_t nbins, Double_t xmin, Double_t xmax) override { if(auto v = dynamic_cast<RooRealVar*>(rvar())) v->setBinning(RooUniformBinning(xmin,xmax,nbins),GetName()); TAxis::Set(nbins,xmin,xmax); }
...
const char* xRooNode::GetIconName() const {
    if (auto o = get(); o) {
        ...
        if(o->InheritsFrom("RooStats::HistFactory::FlexibleInterpVar")) return "TBranchElement-folder";
...
xRooNode xRooNode::Remove(const xRooNode& child) {
    ...
    if (strcmp(GetName(),".factors")==0 || strcmp(GetName(),".constraints")==0 || strcmp(GetName(),".components")==0) {
        ...
        if (auto p = fParent->get<RooProdPdf>(); p) {
            ...
            if (!arg) arg = p2->components().find(child.GetName());
            ...
            auto arg = toRemove.get<RooAbsArg>();
            if (!arg) arg = p4->_funcList.find(child.GetName());
            if (!arg)
                throw std::runtime_error(TString::Format("Cannot find %s in %s", child.GetName(), fParent->GetName()));
            // remove, including coef removal ....
            auto idx = p4->_funcList.index(arg);

            if (idx != -1) {

                p4->_funcList.remove(*arg);
                ...
                for(size_t ii=0; ii < p4->_coefList.size(); ii++) { if(ii != size_t(idx)) _coefs.push_back(p4->_coefList.at(ii)); }
                p4->_coefList.removeAll();
                for(auto& a : _coefs) p4->_coefList.add(*a);
...
xRooNode xRooNode::Add(const xRooNode& child, Option_t* opt) {
    ...
    if (strcmp(GetName(),".factors")==0) {
        ...
        if (auto _ws = ws(); _ws && fParent) {
            ...
            if (auto _d = _ws->data(child.GetName()); _d) {
                ...
                if(auto __d = _ws->data(child.GetName())) __d->SetBit(1<<20, _ws->allData().size()==1);// sets as selected if is only ds
            }
    ...
    if (auto p = get<RooRealSumPdf>(); p) {
        ...
        if (auto _f = std::dynamic_pointer_cast<RooAbsReal>(out); _f) {
            ...
            if (auto _ax = GetXaxis(); _ax && dynamic_cast<RooAbsRealLValue*>(_ax->GetParent())) {
                ...
                if(auto _boundaries = std::unique_ptr<std::list<double>>(_f->binBoundaries(*dynamic_cast<RooAbsRealLValue*>(_ax->GetParent()),-std::numeric_limits<double>::infinity(), std::numeric_limits<double>::infinity())); !_boundaries && _ax->GetNbins()>0) {
                    ...
                    if (!sumPdf->getStringAttribute("alias")) sumPdf->setStringAttribute("alias",out->GetName());
        ...
        if (child.get()) {
            auto out = acquire(child.fComp);
            if(out) return xRooNode(child.GetName(),out,*this);
        }
...
bool xRooNode::IsHidden() const {
    ...
    if (a) return a->getAttribute("hidden");
    return false;
}
...
xRooNode xRooNode::shallowCopy(const std::string& name, std::shared_ptr<xRooNode> parent) {
    xRooNode out(name.c_str(),nullptr,parent /*? parent : fParent -- was passing fParent for getObject benefit before fProvider concept*/);
    ...
    if(auto s = get<RooSimultaneous>(); s) {
        ...
        if (!chans.empty()) {
            ...
            for(auto& c : chans) {
                ...
                auto c_copy = c->shallowCopy(name+"_"+c->get()->GetName(),std::shared_ptr<xRooNode>(&out,[](xRooNode*){}));
...
//                }
        ...
        if(auto p = fParent->fParent->get<RooAddPdf>()) {
            for(size_t i=0;i<p->pdfList().size();i++) {
                if(p->pdfList().at(i)==fParent->get<RooAbsArg>()) {
                    auto coefs = p->_coefList.at(i);
                    if (!coefs->InheritsFrom("RooProduct")) {
                        RooArgList oldCoef; if (!(strcmp(coefs->GetName(),"1")==0 || strcmp(coefs->GetName(),"ONE")==0)) oldCoef.add(*coefs);
                        auto newCoefs = fParent->acquireNew<RooProduct>(TString::Format("coefs_%s",fParent->GetName()),TString::Format("coefficients for %s",fParent->GetName()),oldCoef);
                        RooArgList oldCoefs;
                        for(size_t j=0;j<p->_coefList.size();j++) {
                            if(i==j) oldCoefs.add(*newCoefs);
                            else oldCoefs.add(*p->_coefList.at(j));
                        }
                        p->_coefList.removeAll();
                        p->_coefList.add(oldCoefs);
                        coefs = newCoefs.get();
                    }
    ...
    if(auto p = get<RooProduct>();p) {
        ...
        if (child.get<RooAbsPdf>()) out = acquire(child.fComp);
            ...
            if(!child.get() && strcmp(child.GetName(),"components")==0) {
                ...
                auto _sumpdf = acquireNew<RooRealSumPdf>(Form("%s_%s", p2->GetName(), child.GetName()),
                                                         (strlen(child.GetTitle()) &&
                                                          strcmp(child.GetTitle(), child.GetName())) ? child.GetTitle()
                                                                                                     : p2->GetTitle(),
        ...
        if(child.get<RooAbsReal>()) {
            ...
            Info("Multiply", "Flagged %s as a global factor in channel %s (is applied to all current and future samples in the channel)", out->GetName(), p3->GetName());
            return xRooNode(out,*this);
        }
        ...
        for(auto& arg : p5->clients()) {cl.insert(arg);}
        ...
        if(cl.size()>1) {
            ...
            else { Warning("Multiply", "Scaling %s that has multiple clients", p5->GetName()); }
        }

        auto new_p = acquireNew<RooProduct>(TString::Format("prod_%s", p5->GetName()), p5->GetTitle(), RooArgList(*p5));
        // copy attributes over
        for(auto& a : p5->attributes()) new_p->setAttribute(a.c_str());
        for(auto& a : p5->stringAttributes()) new_p->setStringAttribute(a.first.c_str(), a.second.c_str());
        if (!new_p->getStringAttribute("alias")) new_p->setStringAttribute("alias", p5->GetName());
        auto old_p = p5;
    ...
    if(!child.get() && strlen(opt)==0) return Multiply(child,"norm");
...
xRooNode xRooNode::Vary(const xRooNode& child) {
    ...
    if (auto s = get<RooSimultaneous>(); s) {
        ...
        if (!f) throw std::runtime_error(TString::Format("Interpolating %s instead of RooHistFunc", p2->_nominal.absArg()->ClassName()));
        RooHistFunc* nomf = f;
        ...
        //child.convertForAcquisition(*this);
        if(f) {
            if(child.get()) xRooNode("tmp", *f, *this) = *child.get();
            f->setValueDirty();
            xRooNode out(*f,*this);
            out.sterilize();
            return out;
        ...
        // get the list of clients BEFORE creating the new interpolation ... seems list of clients is inaccurate after
        std::set<RooAbsArg*> cl;
        for(auto& arg : p3->clients()) {cl.insert(arg);}
        // if multiple clients, see if only one client is in parentage route
        // if so, then assume thats the only client we should replace in
        if(cl.size()>1) {
            if(cl.count( fParent->get<RooAbsArg>() )>0) { cl.clear(); cl.insert(fParent->get<RooAbsArg>()); }
            else { Warning("Vary", "Varying %s that has multiple clients", p3->GetName()); }
        }
        p3->setStringAttribute("origName", p3->GetName());
        TString n=p3->GetName();p3->SetName(Form("%s_nominal", p3->GetName())); // if problems should perhaps not rename here

        auto new_p = acquireNew<RooStats::HistFactory::FlexibleInterpVar>(n, p3->GetTitle(), RooArgList(), p3->getVal(), std::vector<double>(),
                                                                          std::vector<double>());

        // copy attributes over
        for(auto& a : p3->attributes()) new_p->setAttribute(a.c_str());
        for(auto& a : p3->stringAttributes()) new_p->setStringAttribute(a.first.c_str(), a.second.c_str());
        ...
        // get the list of clients BEFORE creating the new interpolation ... seems list of clients is inaccurate after
        std::set<RooAbsArg*> cl;
        for(auto& arg : p4->clients()) {cl.insert(arg);}
        // if multiple clients, see if only one client is in parentage route
        // if so, then assume thats the only client we should replace in
        if(cl.size()>1) {
            if(cl.count( fParent->get<RooAbsArg>() )>0) { cl.clear(); cl.insert(fParent->get<RooAbsArg>()); }
            else { Warning("Vary", "Varying %s that has multiple clients", p4->GetName()); }
        }
        p4->setStringAttribute("origName", p4->GetName());
        TString n=p4->GetName();p4->SetName(Form("%s_nominal", p4->GetName())); // if problems should perhaps not rename here


        auto new_p = acquireNew<PiecewiseInterpolation>(n, p4->GetTitle(), *p4, RooArgList(), RooArgList(), RooArgList());

        // copy attributes over
        for(auto& a : p4->attributes()) new_p->setAttribute(a.c_str());
        for(auto& a : p4->stringAttributes()) new_p->setStringAttribute(a.first.c_str(), a.second.c_str());
        //if (!new_p->getStringAttribute("alias")) new_p->setStringAttribute("alias",p->GetName());
        auto old_p = p4;
        new_p->setAttribute(Form("ORIGNAME:%s",old_p->GetName())); // used in redirectServers to say what this replaces
        for(auto arg : cl) {
...
std::shared_ptr<TObject> xRooNode::convertForAcquisition(xRooNode& acquirer, const char* opt) const {
    ...
    if (sOpt=="func") sName = TString("factory:") + sName;
    ...
    if(auto h = get<TH1>(); h) {
        ...
        TString s(sName); s = TString(s(8,s.Length()));
    ...
    if (_ws &&  (get()==_ws || _ws->arg(GetName()) || (arg && strcmp(arg->GetName(),GetName())==0))) {
        ...
        if (auto a = dynamic_cast<RooAbsArg*>(arg.get()); a) {
            ...
            if (!out_arg){
                ...
                for(auto& aa : _ws->_namedSets) {
                    if (TString(aa.first.c_str()).BeginsWith("CACHE_")) { setNames.insert(aa.first); }
                }
                for(auto& aa : setNames) ws()->removeSet(aa.c_str());
...
TGListTree* xRooNode::GetListTree(TBrowser* b) const {
    if (!b) {
        for (auto o: *gROOT->GetListOfBrowsers()) {
            ...
            if (!b || !b->GetBrowserImp()) continue;
            if (auto out = GetListTree(b); out) return out;
        }
        return nullptr;
    }
    if(b->GetBrowserImp()){
        if(auto _b = dynamic_cast<TGFileBrowser*>( dynamic_cast<TRootBrowser*>(b->GetBrowserImp())->fActBrowser ); _b) {
            auto _root = _b->fRootDir;
            if (!_root) _root = _b->fListTree->GetFirstItem();
            if (auto item = _b->fListTree->FindItemByObj(_root,const_cast<xRooNode*>(this)); item) {
                return _b->fListTree;
            }
        }
    }
...
xRooNode xRooNode::vars() const {
    ...
    if (auto p = get<RooAbsArg>();p) {
        ...
        if (auto _globs = find(".globs"); _globs && _globs->get<RooAbsCollection>()) {
            ...
            if (auto _globs2 = dynamic_cast<RooArgSet*>(_ws->_snapshots.find(p2->GetName())); _globs2) {
                for(auto a : *_globs2) {
                    a->setAttribute("obs");a->setAttribute("global");
                    out.emplace_back(std::make_shared<xRooNode>(*a,*this));
                    out.get<RooArgList>()->add(*a);
...
//        }
        ...
        for(auto& [k,v] : p5->_namedSets) { // warning -- if this is a map, will it invalidate the object addresses on fill?
        ...
        std::unique_ptr<TIterator> iter(p5->_snapshots.MakeIterator() );
        RooArgSet* snap ;
...
xRooNode xRooNode::coefs() const {
    ...
    xRooNode out(".coefs",coefs.empty() ? nullptr : std::make_shared<RooAddition>(".coefs", "Coefficients of",
                                                     coefs),*this);
...
xRooNode xRooNode::factors() const {
    ...
    if (auto p = get<RooProdPdf>(); p) {
        ...
        else if (auto a2 = _main.get<RooAddPdf>(); a2 && !a2->getStringAttribute("alias")) a2->setStringAttribute("alias", "components");
...
xRooNode xRooNode::variations() const {
    ...
    if (auto p = get<RooSimultaneous>(); p) {
        for(auto& c : p->indexCat()) {
            ...
            if (!pp) continue;
            out.emplace_back(std::make_shared<xRooNode>(TString::Format("%s=%s",p->indexCat().GetName(),c.first.c_str()),*pp,*this));
        }
    } else if (auto p2 = get<PiecewiseInterpolation>(); p2) {
        ...
        for(size_t i=0; i < p2->paramList().size(); i++) {
            ...
            out.emplace_back(std::make_shared<xRooNode>(TString::Format("%s=1", p2->paramList().at(i)->GetName()), *p2->highList().at(i), *this));
            out.emplace_back(std::make_shared<xRooNode>(TString::Format("%s=-1", p2->paramList().at(i)->GetName()), *p2->lowList().at(i), *this));
        }
...
//    if (!_frt) {
//        t = std::make_shared<RooFitResultTree>(std::dynamic_pointer_cast<TTree>(const_cast<xRooNode*>(this)->acquire(t->fTree)).get());
//    }
...
//    auto _data = t->generate(_fr,expected);
//    if (!_frt) {
//        t = std::make_shared<RooDataTree>(std::dynamic_pointer_cast<TTree>(const_cast<xRooNode*>(this)->acquire(t->fTree)).get());
//    }
//    xRooNode parent(_fr ? _fr->GetName() : "unknown",nullptr,xRooNode(t,*this));
//    xRooNode out(_data.first->GetName(),/*acquire(_fr)*/ _data.first,parent);
//    out.emplace_back(std::make_shared<xRooNode>(".globs",std::const_pointer_cast<RooArgSet>(_data.second),out));
...
TH1* xRooNode::BuildHistogram(RooAbsLValue* v, bool empty, bool errors, int binStart, int binEnd) const {
    ...
    if (x) {
        if (x==rar) {
            ...
            h->SetMaximum(x->hasMax() ? x->getMax() : (h->GetBinContent(1) + std::max(abs(h->GetBinContent(1)*0.1),50.)));
            h->SetMinimum(x->hasMin() ? x->getMin() :(h->GetBinContent(1) - std::max(abs(h->GetBinContent(1)*0.1),50.)));
    ...
    if (!empty) {
        ...
        for (int i = std::max(1,binStart); i <= std::min(h->GetNbinsX(),binEnd); i++) {
            ...
            if (!warned && (lapTimes.at(0) > 10 || (lapTimes.size()>2 && time_estimate  > 60.) )) {
                TTimeStamp t2; t2.Add(time_estimate);
...
//            }
        ...
        for(auto& _v : _channels) {
            ...
            if (!inRange || !_v->get<RooAbsReal>()->isSelectedComp()) gPad->SetFillColor(kGray);
    ...
    if (rar==vv && rar->IsA()==RooRealVar::Class()) {
        ...
        _hist->GetListOfFunctions()->Add(
                new TExec(".update",
                          TString::Format("gROOT->SetEditHistograms(true);auto h = dynamic_cast<TH1*>(gPad->GetPrimitive(\"%s\")); if(h) { if(auto n = dynamic_cast<xRooNode*>(h->GetListOfFunctions()->FindObject(\"%s\")); n && n->TestBit(TObject::kNotDeleted) && n->get<RooRealVar>()->getVal() != h->GetBinContent(1)) {double range = h->GetMaximum()-h->GetMinimum(); h->SetBinContent(1, TString::Format(\"%%.2g\",int(h->GetBinContent(1)/(range*0.01))*range*0.01).Atof());n->SetContents( h->GetBinContent(1) ); for(auto pp : *h->GetListOfFunctions()) if(auto hh = dynamic_cast<TH1*>(pp))hh->SetBinContent(1,h->GetBinContent(1));} gPad->Modified();gPad->Update(); }",_hist->GetName(),node->GetName()))
        );
        if (errHist) {
...
std::pair<double,double> xRooNode::IntegralAndError(const xRooNode& fr, const char* rangeName) const {
    ...
    if (auto p = dynamic_cast<RooAbsPdf*>(get()); p) {
        ...
        auto f = std::shared_ptr<RooAbsReal>(p2->createIntegral(*std::unique_ptr<RooArgSet>(p2->getObservables(_obs)), rangeName)); // did use x here before using obs
